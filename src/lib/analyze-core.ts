...
■ 正しい出力例（文体・分量の見本。キーワード名・数値はすべて架空であり、実際の出力には必ず上の【】ブロック内のキーワード・数値だけを使うこと）
{
  ...
  "reviewCountComment": "累計件数は<strong>着実に増加を続けており</strong>、集客の資産として積み上がっている。直近は伸びがやや緩やかになっているため、来店客への声かけなど獲得のきっかけづくりを継続したい",
...
- **他店との比較は禁止（厳守）**: 「同業種平均」「グループ平均」「他店平均」「業界平均」「全国平均」
  「同規模店と比較して」「平均を上回る／下回る」など、**自店以外の水準を引き合いに出す表現は一切書かない**。
