...
■ 正しい出力例（文体・分量の見本。キーワード名・数値はすべて架空であり、実際の出力には必ず上の【】ブロック内のキーワード・数値だけを使うこと）
{
  ...
  "mapComment": "<strong>マップ表示が前月比+5%</strong>と回復傾向にある。ただし同業種8店舗の平均を下回る水準が続いており、露出を伸ばす余地は大きい。写真の定期更新や最新情報の投稿など、プロフィールの鮮度を保つ運用が引き続き有効といえる",
...
- mapComment/searchComment/reactionComment: ${hasKpi ? "各KPIの前月比傾向について2〜3文で。【同業種の平均】ブロックが提供されている場合のみ、そこに書かれた母数を添えて「同業種◯店舗の平均を上回る」のように比較してよい" : "口コミから推定した概況を2〜3文で"}。絶対値（147,422回等）は書かない
- **比較平均の扱い（厳守）**: 「同業種平均」「グループ平均」「他店平均」「業界平均」「全国平均」等の比較表現は、
  【同業種の平均】ブロックが提供されているときだけ使ってよい。提供されていなければ一切書かない（推測禁止）。
  使うときは必ず母数（「同業種5店舗の平均」等）を書く。この平均は弊社が運用支援している店舗の実績値であり、
  業界統計ではないため「業界平均」「全国平均」とは絶対に書かない。「グループ平均」という語も使わない
