...
export function buildPmaxAdvice(input: PmaxAdviceInput): string[] {
  ...
  if (hasPrevMonth && impressions < prevImpressions * 0.97 && prevCtr > 0 && ctr > prevCtr) {
    paragraphs.push(
  ...
  const namedLangs = langCpcs
    ...
    .map((l) => ({ label: LANG_LABELS[l.language] || l.language, cpcYen: l.cpcYen }));
  ...
  // ⑥ 保存・共有の増加（検討中の見込み客）
  if (saveShare > 0 && saveShare > prevSaveShare) {
    paragraphs.push(
