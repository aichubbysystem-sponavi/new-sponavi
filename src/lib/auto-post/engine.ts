/**
 * シート自動投稿のエンジン（HTTP層から独立）
 *
 * 2026-09-04: 以前は /api/report/auto-post/route.ts がこの処理を丸ごと持ち、ブラウザが
 * 10店舗ずつ最大300秒のリクエストを繰り返していた。80店舗の即時実行がバッチ1で
 * タイムアウトし、残り70店舗が未処理のまま「完了」になったため、処理本体をここへ移し
 *   - /api/report/auto-post（プレビュー等・互換用）
 *   - /api/cron/auto-post-worker（ジョブ方式のバックグラウンド実行）
...
/** GBPが sourceUrl から取得できる上限（image-proxy の MAX_VIDEO_BYTES と同値） */
const MAX_VIDEO_BYTES = 25 * 1024 * 1024;

/** Dropboxから取り出したメディア。動画対応のためURLとファイル名を対で持つ */
type MediaItem = { url: string; name: string; webUrl?: string; bytes?: number };
...
function dropboxWebUrl(rootPathLower: string, relPath: string): string {
  ...
  // encodeURIComponent は ( ) を素通しするが、画面のリンク化は ) でURLを終端させるため明示的にエンコードする
  // （「写真投稿26-8-4 (1).jpg」は全ファイルがこの形式）
  const enc = (s: string) => encodeURIComponent(s).replace(/\(/g, "%28").replace(/\)/g, "%29");
  return `https://www.dropbox.com/home${dir.split("/").map(enc).join("/")}?preview=${enc(file)}`;
}
...
async function dropboxFetch(url: string, init: RequestInit): Promise<Response> {
  ...
  for (let attempt = 1; attempt <= 3 && res.status === 429; attempt++) {
    ...
    // 呼び出し側の AbortSignal.timeout はタイマーが走り続けているので、待った後に使い回すと
    // 既にabort済みで即失敗する。再試行ごとに新しいタイムアウトを張る
    res = await fetch(url, { cache: "no-store" as const, ...init, signal: AbortSignal.timeout(20000) });
  }
  return res;
}
...
async function searchDropboxPhotosMultiple(folderUrl: string, dateCompact: string, shopName: string): Promise<{ items: MediaItem[]; debug: string }> {
  ...
  try {
    ...
    try {
      ...
      files.push(...root.files.map((e: any) => ({ name: e.name || "", path: e.path_display || e.path_lower || "", bytes: Number(e.size) || 0 })));
      ...
      while (pendingFolders.length > 0) {
        ...
        try {
          ...
          files.push(...sub.files.map((e: any) => ({ name: e.name || "", path: e.path_display || e.path_lower || "", bytes: Number(e.size) || 0 })));
          // さらに深いサブフォルダがあれば追加
    ...
    for (const file of dateMatches.slice(0, 10)) {
      ...
      try {
        ...
        if (linkRes.ok) {
          ...
          if (linkData.link) { items.push({ url: linkData.link, name: file.name, webUrl: dropboxWebUrl(sharedRootPath, file.path), bytes: file.bytes }); got = true; }
        }
      ...
      if (!got && sharedRootPath) {
        ...
        try {
          ...
          if (linkRes2.ok) {
            ...
            if (linkData2.link) { items.push({ url: linkData2.link, name: file.name, webUrl: dropboxWebUrl(sharedRootPath, file.path), bytes: file.bytes }); got = true; }
          }
      ...
      if (!got) {
        ...
        try {
          ...
          if (fileShareUrl) {
            items.push({ url: fileShareUrl.replace("www.dropbox.com", "dl.dropboxusercontent.com").replace(/\?dl=0/, "?dl=1"), name: file.name, webUrl: dropboxWebUrl(sharedRootPath, file.path), bytes: file.bytes });
            got = true;
          }
...
async function searchDropboxByShopName(shopName: string, dateCompact: string): Promise<{ items: MediaItem[]; debug: string }> {
  ...
  try {
    ...
    const toFile = (e: any) => ({ name: e.name || "", path: e.path_display || e.path_lower || "", bytes: Number(e.size) || 0 });
    ...
    for (const file of dateMatches.slice(0, 10)) {
      ...
      try {
        ...
        if (linkRes.ok) {
          ...
          if (d.link) { items.push({ url: d.link, name: file.name, webUrl: dropboxWebUrl(sharedRootPath, file.path), bytes: file.bytes }); got = true; }
        }
      ...
      if (!got && sharedRootPath) {
        ...
        try {
          ...
          if (linkRes2.ok) {
            ...
            if (d2.link) { items.push({ url: d2.link, name: file.name, webUrl: dropboxWebUrl(sharedRootPath, file.path), bytes: file.bytes }); got = true; }
          }
...
/** 監査ログ用コンテキスト（withAudit の AuditContext と互換。cronワーカーからはダミーを渡す） */
export type AutoPostCtx = { detail?: string; actionOverride?: string };

/** HTTPレスポンス相当。route.ts は json(body, { status }) に、ワーカーはそのまま使う */
export type AutoPostResponse = { status: number; body: any };
const json = (body: any, init?: { status?: number }): AutoPostResponse => ({ status: init?.status || 200, body });

export type AutoPostOptions = {
  /** タブ名→CSV。ワーカーが同じシートを何十回も読む場合に1回の取得を使い回す（未取得タブはここに追記される） */
  csvCache?: Map<string, string | null>;
};

export async function runAutoPost(body: AutoPostBody, ctx: AutoPostCtx, opts: AutoPostOptions = {}): Promise<AutoPostResponse> {
  const { sheetId, targetDate, dryRun, topicType, batchOffset, batchSize, filterShopName, filterShopNames, scheduleMode, scheduleAt, checkOnly } = body;
  ...
  if (!sheetId || !targetDate) {
    return json({ error: "sheetIdとtargetDateが必要です" }, { status: 400 });
  }
  ...
  const csvByTab = await Promise.all(tabs.map(async (tab) => {
    try {
      // ワーカーからの連続呼び出しでは取得済みCSVを使い回す（取得失敗=null はキャッシュせず次回また試す）
      const cached = opts.csvCache?.get(tab);
      if (cached) return { tab, csvText: cached };
      ...
      if (csvText && opts.csvCache) opts.csvCache.set(tab, csvText);
      return { tab, csvText };
    } catch (e) {
  ...
  if (photoSearchTargets.length > 0) {
    ...
    for (let bi = 0; bi < photoSearchTargets.length; bi += PHOTO_BATCH) {
      ...
      const applyItem = (target: any, i: number) => {
        ...
        target.mediaBytes = mediaItems[i].bytes || 0;
  ...
  if (scheduleMode) {
    ...
    const prepare = async (match: (typeof batchMatches)[number]): Promise<Prepared> => {
      ...
      if (match.photoUrl && match.mediaBytes && match.mediaBytes > MAX_VIDEO_BYTES && match.mediaFormat === "VIDEO") {
        ...
        return {
          match, shop, warnings,
          skip: { reason: "動画サイズ超過（スキップ）", detail: `動画が大きすぎます（${mb}MB > Googleが取得できる上限25MB）。720p・30秒以内に圧縮して25MB以下にしてから入れ直してください / ファイル: ${match.mediaFileName || "?"}${match.mediaWebUrl ? ` ${match.mediaWebUrl}` : ""}` },
        };
      }
    ...
    for (const { match, shop, warnings, skip } of prepared) {
      ...
      try {
        ...
        if (insertErr) {
          // 23505 = 部分一意インデックス（同一店舗・同一予約時刻の pending/on_hold/processing）に当たった。
          // クライアントがタイムアウト後に同じバッチを再送し、前のリクエストがまだ走っている場合に起きる。
          // select→insert の重複チェックはこの競合を防げないので、DB側の制約で二重登録を止める
    ...
    if (skips.length > 0) {
      try {
        // 同じ回・同じ店舗の記録は入れ替える（バッチ再送で同じスキップが二重に数えられないように）
        await supabase.from("auto_post_skips").delete()
          .eq("scheduled_at", scheduledTime).eq("target_date", targetDate)
          .in("shop_name", Array.from(new Set(skips.map((k) => k.shop_name))));
  ...
  if (!accessToken) return json({ error: "OAuthトークンなし" }, { status: 500 });
