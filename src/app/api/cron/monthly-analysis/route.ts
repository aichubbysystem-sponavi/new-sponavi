...
comments[0]「数値分析」: 口コミ動向から推定した概況を箇条書き厳密に3項目。各「・」で始め1文で完結
comments[1]「口コミ傾向と強み」: 高評価で繰り返し言及されるポイントを箇条書き3項目。「〜という声が多い」の形で引用。低評価傾向があれば+1項目（合計4つまで）。各1文で完結
comments[2]「改善策」: 具体的アクションを a) b) c) で3つ。各1文で完結。今日から実行できる施策のみ

■ 制約
- 各項目は必ず主語+述語のある完全な文。単語や体言止めは禁止（✕「・韓国語」→ ○「・韓国語の口コミでも高評価が多い」）
- 箇条書きのみ。長文段落は禁止
- 口コミの「件数」「○○件」「ゼロ」には言及しない。質と傾向のみ
