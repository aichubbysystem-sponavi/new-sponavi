...
${langSection ? "- 口コミ言語は上記集計に記載された言語のみ言及" : ""}`;
      ...
      const { error: upsertErr } = await supabase.from("report_analysis").upsert({
        ...
        comments: (() => {
          const clean = (s: string) => s.replace(/。$/, "").replace(/[\u200B-\u200D\uFEFF\u00AD\uFFFD]/g, "").replace(/\s{2,}/g, " ").trim();
          const analysis: string[] = (parsed.analysis || []).map(clean).filter((s: string) => s.length >= 10);
          const reviews: string[] = (parsed.reviews || []).map(clean).filter((s: string) => s.length >= 10);
          const actions: string[] = (parsed.actions || []).map(clean).filter((s: string) => s.length >= 10);
          if (analysis.length || reviews.length) {
            return [
