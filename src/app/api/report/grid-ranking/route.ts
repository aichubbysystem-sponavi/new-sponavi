...
export const POST = withAudit("多地点順位実測", "PAID_OP", async (request, ctx) => {
  ...
  //
  // 格子幅は「計測距離と同じ幅、上限500m」（gridFactor = intervalM/1000 を 0.1〜0.5 にclamp）。
  // 格子が計測距離より粗いと外周点が中心と同一セルに丸まり「5地点全部が同じキャッシュ順位」
  // になるため、下限は必ず最小インターバル（100m=0.1）以下に保つこと。
  //
  // 下限を0.25→0.1に変更（2026-08-10 距離100m/300m追加対応）:
  // 250m格子のままだと100m計測の外周点（軸方向オフセット≈71m）が全て中心セルに丸まり
  // 計測が機能しなかった。500m設定（既存全店舗）は従来どおり factor 0.5 のままなので
  // 既存キャッシュキーへの影響なし。
  // 上限0.5の経緯（2026-08-02）: 格子1kmだと1km/1.5km/2kmの外周点が同一格子に丸まった。
  // 格子500mでも隣接する刻み同士（例: 700m↔1km）がごく稀に（実測1〜3%）同一セルに
  // 落ちることは残る。UI注記済み。
  const intervalM = Number(interval) || 1000;
  const gridFactor = Math.min(0.5, Math.max(0.1, intervalM / 1000));
