...
export const POST = withAudit("多地点順位実測", "PAID_OP", async (request, ctx) => {
  ...
  //
  // 【現状】許可インターバルは500〜4000mなので intervalM/1000 は常に0.5以上となり、
  // gridFactorは事実上0.5固定＝格子は約500m四方（LAT_STEP*0.5, LNG_STEP*0.5）。
  // clampの形を残しているのは、将来250m等の細かい刻みを追加したときに
  // 「格子が計測間隔より粗くて刻みが消える」事故を自動で防ぐため（下限0.25=250m格子）。
  //
  // 上限を1.0→0.5に変更（2026-08-02 距離500m刻み対応）:
  // 格子1kmのままだと、斜め配置の外周点は距離1km/1.5km/2kmで同一格子に丸まり、
  // 新設した500m刻みの選択肢が計測に反映されなかった（斜めの軸方向オフセット差=約354m/500m刻み）。
  // 格子500mでも隣接する刻み同士がごく稀に（実測1〜3%）同一セルに落ちることは残る。UI注記済み。
  // 変更時点で1km以上を設定した店舗はゼロ（DB実測）のため、既存キャッシュキーへの影響なし
