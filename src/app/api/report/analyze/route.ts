...
【出力形式】以下のJSON形式のみ出力してください。WordSourcesは不要です。
{
  ...
  "comments": [
    ...
    "コメント2: 口コミ・集客分析（評価はGoogle公式${averageRating}点を使用。口コミ数を同業種平均と比較する際は累計${totalReviewCount}件を使え。直近1年${totalRated}件は分析母数であり累計ではない。評価分布、口コミ増加ペース、検索語句の指名/一般比率、キーワード順位変動を総合分析）",
