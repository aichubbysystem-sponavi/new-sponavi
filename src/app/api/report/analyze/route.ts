...
【出力形式】以下のJSON形式のみ出力してください。WordSourcesは不要です。
{
  ...
  "comments": [
    ...
    "コメント5: 来月の具体的な施策提案（${hasKpi ? "KPI改善（検索数回復・アクション率向上）、順位対策、" : ""}口コミ促進の各面から実行可能なアクション3〜4つ。各施策はa) b) c) d)で番号付けすること。①②③④は使わない）"
  ]
}
...
- コメント2〜4間で同じキーワード順位やデータを繰り返さない。コメント2で触れた順位変動はコメント3・4では詳細を省略し、新しい切り口で言及する。
- 口コミ増加ペースに言及する際は、提供データの月ラベルをそのまま使うこと。「当月」「今月」等の曖昧な表現は使わない。`;
          ...
          if (reviewDelta && reviewDelta.length > 0 && reviewLabels && reviewLabels.length > 0) {
            ...
            for (let ri = 0; ri < reviewLabels.length; ri++) {
              ...
              if (mMatch) {
                ...
                // 年を推定（reviewLabelsは連続なので、1月に戻ったら翌年）
                let year = baseYear;
                if (ri > 0) {
                  const prevMatch = (reviewLabels[ri - 1] || "").match(/(\d{1,2})月/);
                  if (prevMatch && parseInt(prevMatch[1]) > monthNum) year = baseYear + 1;
                }
                const rNum = year * 100 + monthNum;
