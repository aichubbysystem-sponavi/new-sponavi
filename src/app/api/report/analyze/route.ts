...
【出力形式】以下のJSON形式のみ出力してください。WordSourcesは不要です。
{
  ...
  "comments": [
    "【数値分析】${hasKpi ? "マップ・検索・アクションの前月比を簡潔に。同業種/グループ比較も含める" : "口コミ動向から推定した概況"}。箇条書き（・）で3項目。各項目は1文20文字程度で端的に",
    "【口コミ傾向と強み】口コミで評価されているポイントを引用。順位動向も含める。箇条書き（・）で3項目。各1文",
    "【改善策】実行可能なアクションを a) b) c) で3つ。各1文で簡潔に"
  ]
}

【commentsのルール（厳守）】
- ★簡潔さ最優先★ 各コメントは80〜130文字以内。3コメント合計で300文字以内を厳守。冗長な説明は不要。
