...
comments[0]「数値分析」:
${hasKpi ? "・マップ表示/検索数/アクションの前月比を箇条書き3項目で。同業種やグループとの比較があれば1項目追加" : "・口コミの傾向から推定した概況を箇条書き3項目で"}
・各項目は「・」で始め、1項目1文（25文字以内）

comments[1]「口コミ傾向と強み」:
・高評価口コミで繰り返し言及されるポイントを箇条書き3項目で
・口コミの実際の表現を「〜という声」の形で引用
・低評価の傾向があれば最後に1項目追加
・各項目は「・」で始め、1項目1文（30文字以内）

comments[2]「改善策」:
・データから導かれる具体的アクションを a) b) c) で3つ
・各1文（30文字以内）。店舗が今日から実行できる施策のみ

■ 絶対守るべき制約
1. 各コメントは箇条書き3〜4項目のみ。長文の段落は禁止
2. レポートのグラフ・テーブルに表示済みの数値（147,422回等）をそのまま書かない。「前月比+5%」のように傾向で表現
3. 数値を使う場合は上記KPIデータの値のみ。独自計算・四捨五入は禁止。評価は必ず${averageRating}
4. 口コミの「件数」「○○件」には言及しない。質と傾向のみ分析
5. 口コミ参照番号（#1等）を含めない
