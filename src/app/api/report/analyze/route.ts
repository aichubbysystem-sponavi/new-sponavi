...
これは「口コミ分析」ではなく「MEOレポート全体の総評」です。KPIデータと口コミの両方を分析してください。
...
【出力形式】以下のJSON形式のみ出力してください。WordSourcesは不要です。
{
  ...
  "comments": [
    "【数値分析】見出しから始め、${hasKpi ? "マップ表示数・検索数・アクション数の前月比/前年比を簡潔に。同業種/グループ平均との比較も含める" : "口コミ動向から推定した概況"}。箇条書き（・）で3〜5項目。各項目は1〜2文で端的に。グラフやテーブルで既に見える数値の羅列は不要、分析と洞察のみ記載",
    "【口コミ傾向と強み】見出しから始め、口コミで評価されているポイントを具体的に引用しつつ分析。キーワード順位や検索語句の動向も含める。箇条書き（・）で3〜5項目",
    "【改善策】見出しから始め、実行可能なアクションを a) b) c) d) で3〜4つ提案。各施策は1〜2文で簡潔に。口コミの課題を踏まえた改善も含める"
  ]
}

【commentsのルール（厳守）】
- ★簡潔さ最優先★ 各コメントは200〜350文字以内。冗長な説明は不要。分析と洞察のみ。
- これはお客様に見せるMEOレポートの総評です。グラフやテーブルで既に表示している数値をそのまま繰り返さない。
- 箇条書き（・）を使って構造化すること。長文の段落は禁止。
${hasKpi ? `- 数値を引用する場合は「マップ表示数は前月比+5%と回復基調」のように傾向と合わせて端的に。` : "- KPIデータが未取得のため、口コミデータを中心に分析してください（その旨は書かない）。"}
- 数値は必ず上記の統計データ・KPIデータの値をそのまま使用。独自計算は禁止。評価は${averageRating}。
- 口コミの「件数」「○○件」には一切言及しないこと。口コミの質・傾向のみ分析。
- コメント内に口コミ参照番号（#1, #6等）を絶対に含めない。
- 具体的な口コミ引用は「○○という声がある」のように表現。
- 重要な数値や結論は<strong>タグで強調。
- 改善策では、店舗が実際に行っていない具体的なキャンペーン・特典・割引を捏造しないこと。
- ★データ重複禁止★ 同じ数値・同じ比較を複数のコメントで繰り返すことは厳禁。
- 口コミ増加ペースに言及する際は、提供データの月ラベルをそのまま使うこと。
${langStatsText ? "- ★口コミ言語★ 上記【口コミ言語別集計】に記載された言語のみ言及すること。集計に含まれない言語を推測で記述することは厳禁。" : ""}`;
