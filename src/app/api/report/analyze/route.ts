...
【出力形式】以下のJSON形式のみ出力してください。WordSourcesは不要です。
{
  ...
  "comments": [
    "【数値分析】${hasKpi ? "マップ表示数・検索数・アクション数の前月比/前年比を簡潔に。同業種/グループ平均との比較も含める" : "口コミ動向から推定した概況"}。箇条書き（・）で3〜4項目。各項目は1文で端的に",
    "【口コミ傾向と強み】口コミで評価されているポイントを引用しつつ分析。キーワード順位の動向も含める。箇条書き（・）で3〜4項目",
    "【改善策】実行可能なアクションを a) b) c) で3つ提案。各施策は1文で簡潔に"
  ]
}

【commentsのルール（厳守）】
- ★簡潔さ最優先★ 各コメントは150〜250文字以内。冗長な説明は不要。分析と洞察のみ。3コメント合計で600文字以内を厳守。
