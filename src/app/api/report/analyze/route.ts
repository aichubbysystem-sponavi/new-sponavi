...
async function tryAnalyze(
  ...
  const prompt = `あなたはMEO対策のプロコンサルタントです。クライアント店舗への月次レポートに掲載する「総評」を作成してください。
読み手はMEOの専門知識を持たない店舗オーナーです。専門用語は避け、平易な日本語で書いてください。

■ 店舗情報
店舗名: ${shopName}
Google評価: ${averageRating} / 5.0
...
{
  "positiveWords": ["原文そのままの短いフレーズ12個以上"],
  "negativeWords": ["原文そのままの短いフレーズ12個以上"],
  "summary": "50文字以内の1行総評",
  "comments": ["コメント1", "コメント2", "コメント3"]
}

■ positiveWords / negativeWords のルール
- 口コミ原文に書かれている表現をそのまま抜き出す。言い換え・意訳は禁止
- 例: 原文「スープが熱々で美味しかった」→ ○「スープが熱々」 ✕「温かいスープ」
- 各2〜8文字の短いフレーズ。positive=★4-5、negative=★1-3 から抽出

■ comments のルール（最重要）
comments配列は必ず3要素。各コメントの構成:

comments[0]「数値分析」:
${hasKpi ? "・マップ表示/検索数/アクションの前月比を箇条書き3項目で" : "・口コミの傾向から推定した概況を箇条書き3項目で"}
・各項目は「・」で始め、1項目1文で完結させる
・項目数は厳密に3つ。4つ以上は禁止

comments[1]「口コミ傾向と強み」:
・高評価口コミで繰り返し言及されるポイントを箇条書き3項目で
・口コミの実際の表現を「〜という声が多い」の形で引用
・低評価の傾向があれば最後に1項目追加（合計4項目まで）
・各項目は「・」で始め、1項目1文で完結させる
・★重要★ 単語だけの項目は禁止（✕「・韓国語」→ ○「・韓国語の口コミでも高評価が多い」）

comments[2]「改善策」:
・データから導かれる具体的アクションを a) b) c) で3つ
・各1文で完結。店舗が今日から実行できる施策のみ

■ 絶対守るべき制約
1. 各項目は必ず主語+述語のある完全な文。単語や体言止めは禁止
2. レポートのグラフ・テーブルに表示済みの絶対値（147,422回等）をそのまま書かない。「前月比+5%」のように傾向で表現
3. 数値を使う場合は上記KPIデータの値のみ。独自計算・四捨五入は禁止。評価は必ず${averageRating}
4. 口コミの「件数」「○○件」「ゼロ」「0件」には一切言及しない。口コミの質と傾向のみ分析
5. 口コミ参照番号（#1等）を含めない
6. 改善策で店舗が行っていないキャンペーン・特典・割引を捏造しない
7. 同じ数値・同じ比較を複数コメントで繰り返さない
8. <strong>タグで重要な結論を1コメントにつき1〜2箇所だけ強調
${langStatsText ? "9. 口コミ言語は上記【口コミ言語別集計】に記載された言語のみ言及。推測で言語を追加しない" : ""}`;
      ...
      if (Array.isArray(parsed.comments)) {
        parsed.comments = parsed.comments.map((c: string) =>
          c
            ...
            .replace(/[\u200B-\u200D\uFEFF\u00AD]/g, "")   // ゼロ幅文字・制御文字除去
            .replace(/[\uFFFD]/g, "")                       // 置換文字(文字化け)除去
            .replace(/\s{2,}/g, " ")
            .trim()
        );
      }
