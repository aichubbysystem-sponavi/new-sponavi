...
■ 正しい出力例
{
  ...
  "reviews": [
    "「味噌のコクがたまらない」と味への満足度が高い",
    "「駅直結で便利」と立地を評価する声が多い",
    "海外からの好意的な口コミも見られる",
    "接客態度への不満が低評価の主因となっている"
  ],
  "actions": [
...
■ ルール
- analysis: ${hasKpi ? "KPI前月比の傾向を3項目" : "口コミから推定した概況を3項目"}。絶対値（147,422回等）は書かない
...
${langStatsText ? "- 口コミ言語は上記集計に記載された言語のみ言及" : ""}`;
  ...
  try {
    ...
    try {
      ...
      const analysis: string[] = (parsed.analysis || []).map(cleanItem).filter((s: string) => s.length >= 10);
      const reviews: string[] = (parsed.reviews || []).map(cleanItem).filter((s: string) => s.length >= 10);
      const actions: string[] = (parsed.actions || []).map(cleanItem).filter((s: string) => s.length >= 10);
      ...
      // 旧形式のcommentsフォールバック（analysis/reviews/actionsが空の場合）
      if (!analysis.length && !reviews.length && Array.isArray(parsed.comments_raw || parsed.comments_orig)) {
        parsed.comments = (parsed.comments_raw || parsed.comments_orig).map((c: string) => cleanItem(c));
      }
