...
■ 出力（JSON以外は一切書くな）
{"positiveWords":["原文フレーズ12個"],"negativeWords":["原文フレーズ12個"],"summary":"20文字の総評","comments":["数値分析","口コミ傾向","改善策"]}

■ 正しいcommentsの例（このフォーマットを厳守）
[
  "・<strong>マップ表示が前月比+5%</strong>と回復傾向・ルート検索は+60%と大幅増加・アクション率は前月比+0.24pt改善",
  "・「味噌のコクがたまらない」と<strong>味への満足度が高い</strong>・「駅直結で便利」と立地を評価する声が多い・多言語の好意的な口コミが増加傾向・接客への不満が低評価の主因",
  "a) 接客品質の均一化に向けた声かけルールを策定する b) 荷物置き場の確保で設備面の不満を解消する c) <strong>口コミ投稿を促す卓上POP</strong>を設置する"
]

■ ルール
- comments[0]: 「・」区切りで3項目。KPI前月比の傾向のみ。絶対値は書かない
- comments[1]: 「・」区切りで3〜4項目。口コミ引用は「〜」で囲む。低評価傾向も1項目含める
- comments[2]: a) b) c) の3施策。今日から実行可能な具体策のみ
- 各項目は20文字前後の1文。例文と同程度の短さを厳守
- <strong>は各コメント1〜2箇所のみ
...
${langStatsText ? "- 口コミ言語は上記集計に記載された言語のみ言及" : ""}`;
  ...
  try {
    ...
    try {
      ...
      // 後処理: 各コメントの箇条書き項目を強制トリム
      if (parsed.comments && Array.isArray(parsed.comments)) {
        parsed.comments = parsed.comments.map((c: string, ci: number) => {
          if (!c) return c;
          // 「・」区切りの項目を分割して各項目をトリム
          if (ci <= 1) {
            // comments[0], [1]: 「・」区切り
            const items = c.split(/・/).filter(s => s.trim());
            const maxItems = ci === 0 ? 3 : 4;
            const trimmed = items.slice(0, maxItems).map(item => {
              return item.replace(/。$/, "").trim();
            }).filter(t => {
              if (t.length < 10) return false; // 短すぎる断片を除去
              if (!/[るいたすくだむぶぬえあ向上減少傾向改善増加多]/.test(t.slice(-3))) return false; // 文として不完全な断片を除去
              return true;
            });
            return trimmed.map(t => `・${t}`).join("");
          } else {
            // comments[2]: a) b) c) 区切り
            const items = c.split(/(?=[a-c]\)\s?)/).filter(s => s.trim());
            const trimmed = items.slice(0, 3).map(item => {
              return item.replace(/。$/, "").trim();
            });
            return trimmed.join(" ");
          }
